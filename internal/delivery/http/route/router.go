...
func SetupRoutes(app *fiber.App, h *AppHandlers, tokenMaster *token.TokenMaster, playgroundHandler fiber.Handler, queryHandler fiber.Handler) {
	...
	app.All("/query", middleware.Protected(tokenMaster), queryHandler)
	// -----------------------------AUTH GROUP----------------------------------
	auth := api.Group("/auth")
